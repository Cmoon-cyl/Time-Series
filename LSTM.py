...
class LSTM(nn.Module):
    def __init__(self):
        ...
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.lstm = nn.LSTM(60, 10, batch_first=True)
        nn.Dropout(p=...)
        nn.BatchNorm1d()
...
class Model:
    ...
    def get_k_fold_data(self, i, X, y):
        ...
        # return X_train.view(X_train.shape[0], X_train.shape[1], 1), y_train.view(y_train.shape[0], y_train.shape[1], 1), \
        #        X_valid.view(X_valid.shape[0], X_valid.shape[1], 1), y_valid.view(y_valid.shape[0], y_valid.shape[1], 1)
    ...
    def predict(self):
        ...
        for x in self.dataset.test_features:
            predict.append(self.net(x.view(1,60))[0].detach().to('cpu').numpy())
...
if __name__ == '__main__':
    ...
    data.preprocess()
    trainer = Model(data, num_epochs=500, batch_size=32,
